...
class LocalCostmap(Node):
    ...
    def cloud_callback(self, msg):
        ...
        # 从 PointCloud2 提取点
        points = np.array(list(self.read_points(msg)), dtype=np.float32)
        if points.shape[0] == 0:
            return

        # 获取从点云坐标系到 odom 坐标系的变换
        try:
            ...
            rot_matrix = self.custom_quaternion_matrix([rot.w, rot.x, rot.y, rot.z])[:3, :3]

            # 将点云转换到 odom 坐标系
            points = (rot_matrix @ points.T).T + trans_vec
        except TransformException as e:
            self.get_logger().error(f'TF error in point cloud transform: {e}')
            return
    ...
    def custom_quaternion_matrix(self,quaternion):
        """
        将四元数转换为旋转矩阵。
